...
En_Choice Read_Choice(short &counter) {
  ...
  cout << "\n----------------------------------| Round [" << counter
       << "] |----------------------------------\n";
  do {
    cout << "Please, enter your number of choice (3 for Paper, 2 for Stone, 1 "
            "for Scissor): ";
    cin >> choice;
...
void Final_Result(St_Game_Info game_info) {
  ...
  cout << Taps(2)
       << "--------------------------------------------------------------------"
          "\n";
  cout << Taps(2)
       << "___________________________[Game "
          "Results]___________________________\n";
  cout << Taps(2) << "Game Rounds: " << game_info.rounds << ".\n";
  cout << Taps(2) << "The user won " << game_info.num_user_win
       << Times(game_info.num_user_win);
  cout << Taps(2) << "The computer won " << game_info.num_computer_win
       << Times(game_info.num_computer_win);
